...
export default function EventCard({ event, className = '' }: EventCardProps) {
  const getStatusBadge = () => {
    ...
    if (event.days_until <= 7) {
      return <Badge className="bg-orange-100 text-orange-700 border border-orange-200">{event.days_until} hari lagi</Badge>;
    }
    return null;
  };
    ...
    <div className={`group bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg hover:border-orange-200 transition-all duration-300 ${className}`}>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-medium text-gray-700">{event.formatted_date}</span>
          <Clock className="w-4 h-4 text-orange-500 ml-2" />
          <span className="text-sm text-gray-600">{event.formatted_time}</span>
        </div>
